...
We study about two realizations of the action of the symmetric group $S_n$ for finite natural $n$. 
\begin{itemize}
    \item $S_n$ acts on the index set $X=\ensuremath{\{1,2,\ldots,n\}}$ in the following way: $g\cdot i = g(i)$ for all $g \in S_n$ and $i \in X$.
    \item $S_n$ acts on the vector $v \in \RF^{n}$ by permuting its coordinates: $\left(gv\right)_{i}=v_{g^{-1}(i)}$ for all $g \in S_n$ and $i \in X$.
\end{itemize}
We name the first action as the \textbf{index action} and the second one as the \textbf{coordinate action}.
These actions are equivalent since the the coordinate action is literally the index action applied to the indexes of the vector $v$.
The standard permutation representation of $S_n$  on $\RF^{n}$ is formulated as $\rho: S_n \to GL(n, \RF)$ defined by $\rho(g)_{i,j} = \delta_{j,g(i)}$, where $\delta$ is the Kronecker delta function.

Let $G \leq S_n$ be a subgroup of the symmetric group $S_n$, and lets look on the $G$-linear transformation $T:\RF^{n}\rightarrow\RF^{n}$ that are equivariant with respect to $G$. Thus, we have 
\begin{equation}
T\rho(g)v=\rho(g)Tv
,
\label{eq:equivariantCondition}
\end{equation}
for all $g\in G$ and $v\in\RF^{n}$.

We can solve \eqref{eq:equivariantCondition} by two methodologies:
\begin{itemize}
    \item \textbf{Orbits of Indexes}
    \item \textbf{ Character Theory}
\end{itemize}
...
In the course we have seen the proof that the dimensionality of the space of $G$-linear transformations $HOM_G(\RF^2,\RF^2)$ is equal to 
\begin{equation}
...
Moreover, let $\{O_1, ..., O_k\}$ be the orbits of the action of $G$ on $X^2$, where $k$ is the number of orbits; and let $\oneb_{O_i} \in \RF^{n\times n}$ the indicator matrix of the orbit $O_i$, i.e. $\oneb_{O_i}(i,j) = 1$ if $(i,j) \in O_i$ and $0$ otherwise. 
...
Define  $U^G = \{v \in \RF^{n\times n} \mid (\rho(g) \otimes \rho(g)) v = v, \text{for all} g \in G\}$, as the fixed subspace of $\RF^{n\times n}$ under the action of $G$. In the course we saw that the dimension of this subspace is equal to $\dim HOM_G(\RF^n,\RF^n)$, and is equal to $\tr(\pi_{U^G})$, where $\pi_{U^G}$ is the projection operator onto $U^G$. We showed that $\pi_{U^G} = \frac{1}{|G|} \sum_{g \in G} \rho(g) \otimes \rho(g)$, and therefore we have
\begin{equation}
...
It is clear that each orbit corresponds to a unique eigenvalue of $\rho(g) \otimes \rho(g)$, but the other way need to be explained. If we have an eigenvalue $\lambda$ of $\rho(g) \otimes \rho(g)$, it must satisfy \eqref{eq:indexCondition}, which means that it must be constant on the orbits, therefore it can be represented as a linear combination of the $\{O_i\}_{i=1}^k$, and therefore and therefore a combination of the eigenvalues of $\rho(g) \otimes \rho(g)$ that correspond to $\{O_i\}_{i=1}^k$. So the dimensionality of the space of $G$-linear transformations is equal in both cases.
